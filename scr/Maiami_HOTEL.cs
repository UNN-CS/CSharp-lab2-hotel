...
namespace hotel_maiami
{
    public partial class MAIAMI_HOTEL : Form
    {
        ...
        private void MAIAMI_HOTEL_Load(object sender, EventArgs e)
        {
            ...
            if (File.Exists("F:\\С#\\hotel-maiami\\PriceList.json") == true) //требуется полный путь!!!!
            {
                string stringList = File.ReadAllText("F:\\С#\\hotel-maiami\\PriceList.json");
                price_list PRICELIST = JsonConvert.DeserializeObject<price_list>(File.ReadAllText("F:\\С#\\hotel-maiami\\PriceList.json"));
            ...
            if (File.Exists("F:\\С#\\hotel-maiami\\DopiList.json") == true) //требуется полный путь!!!!
            {
                string stringList = File.ReadAllText("F:\\С#\\hotel-maiami\\DopiList.json");
                dopi_list DOPILIST = JsonConvert.DeserializeObject<dopi_list>(File.ReadAllText("F:\\С#\\hotel-maiami\\DopiList.json"));
        ...
        {
            string comboName = comboBox1.SelectedItem.ToString(); //ситывание name из комбобокса
            double comboPrice = 0;
